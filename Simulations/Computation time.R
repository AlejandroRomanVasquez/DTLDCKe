...
Continuous_to_binary <- function(X, column_type){
  ...
  for(i in 1:p) {   
    if (column_type[i]=="num"){
      ...
      X_mixed[,i] <- ifelse(X[,i]<0,0,1)
    } 
  }
  return(X_mixed)
}

#Continuous to binary by quantile
...
##### Simulations changing p #####

l_nsim <- 5

vec_p_nonDTLCKe_time <- rep(0,l_nsim )
vec_p_parDTLCKe_time <- rep(0,l_nsim )
vec_p_second_order_time <- rep(0,l_nsim )
vec_p_sequential_time <- rep(0,l_nsim )

vec_p <- rep(0,l_nsim)

#n_cores <- 20 # Parallel computing
n_cores <- 1 # Parallel computing
...
# Initial setup
cl_non <- makeCluster(n_cores)
registerDoParallel(cl_non) # Registering a parallel backend for doParallel
...
nondvine_time <- as.numeric(difftime(end_time_non, start_time_non, units = "mins"))

stopCluster(cl_non) #Stopping cluster
print("Time for running the non-DTDCKe PROCEDURE (MIN):")
print(nondvine_time)

vec_p_nonDTLCKe_time[l]<-nondvine_time

# Parametric DTLDCKe #

# Initial setup
cl_par <- makeCluster(n_cores)
registerDoParallel(cl_par) # Registering a parallel backend for doParallel
alpha_dvine <- 0.2 # Target FDR
...
dvine_time <- as.numeric(difftime(end_time_par, start_time_par, units = "mins"))

stopCluster(cl_par) #Stopping cluster
print("Time for running the PAR-DTDCKe PROCEDURE (MIN):")
print(dvine_time)
vec_p_parDTLCKe_time[l] <- dvine_time

# Model-X knockoff second order #
# Initial setup

cl_2nd <- makeCluster(n_cores)
registerDoParallel(cl_2nd) # Registering a parallel backend for doParallel
...
end_time_2nd <- Sys.time()
second_order_time <- as.numeric(difftime(end_time_2nd, start_time_2nd, units = "mins"))
stopCluster(cl_2nd) # Stopping cluster

print("Time for running the SECOND ORDER PROCEDURE (MIN):")
print(second_order_time)

vec_p_second_order_time[l] <- second_order_time

# Sequential knockoffs #

cl_seq <- makeCluster(n_cores)
registerDoParallel(cl_seq) # Registering a parallel backend for doParallel
...
sequential_time <- as.numeric(difftime(end_time_seq, start_time_seq, units = "mins"))
stopCluster(cl_seq) # Stopping cluster

print("Time for running the SEQUENTIAL PROCEDURE (MIN):")
print(sequential_time)
vec_p_sequential_time[l] <- sequential_time

}

#Dataframe with results
df_results_p_varies <- data.frame(Features=rep(vec_p,4), 
                                  Method=c(rep("NonDTLDCKe",l_nsim),rep("DTLDCKe",l_nsim),rep("2nd order",l_nsim),rep("Sequential",l_nsim)),
                                  Time= c(vec_p_nonDTLCKe_time,vec_p_parDTLCKe_time,vec_p_second_order_time,vec_p_sequential_time))

#Exporting the results to csv file
#write.csv(df_results_p_varies, file = "df_results_p_varies_20cores_6dic24.csv", row.names = FALSE)
#write.csv(df_results_p_varies, file = "df_results_p_varies_1core_6dic24.csv", row.names = FALSE)
...
vec_n_nonDTLCKe_time <- rep(0,l_nsim )
vec_n_parDTLCKe_time <- rep(0,l_nsim )
vec_n_second_order_time <- rep(0,l_nsim )
vec_n_sequential_time <- rep(0,l_nsim )
...
for (l in 1:l_nsim ){
  ...
  vec_n_nonDTLCKe_time[l]<-nondvine_time
  ...
  print("Time for running the PAR-DTDCKe PROCEDURE (MIN):")
  print(dvine_time)
  vec_n_parDTLCKe_time[l] <- dvine_time
  ...
  vec_n_second_order_time[l] <- second_order_time
  ...
  print("Time for running the SEQUENTIAL PROCEDURE (MIN):")
  print(sequential_time)
  vec_n_sequential_time[l] <- sequential_time
  
}

#Dataframe with results
df_results_n_varies <- data.frame(Features=rep(vec_n,4), 
                                  Method=c(rep("NonDTLDCKe",l_nsim),rep("DTLDCKe",l_nsim),rep("2nd order",l_nsim),rep("Sequential",l_nsim)),
                                  Time= c(vec_n_nonDTLCKe_time,vec_n_parDTLCKe_time,vec_n_second_order_time,vec_n_sequential_time))



#Exporting the results to csv file
#write.csv(df_results_n_varies, file = "df_results_n_varies_20cores_6dic24.csv", row.names = FALSE)
#write.csv(df_results_n_varies, file = "df_results_n_varies_1core_6dic24.csv", row.names = FALSE)
...
custom_colors <- c("NonDTLDCKe" = "chartreuse4", "DTLDCKe" = "deepskyblue3", 
                         "2nd order" = "orange", "Sequential" = "darkmagenta")

custom_labels <- c("NonDTLDCKe" = "NonDTLDCKe", 
                   "DTLDCKe" = "DTLDCKe", 
                   "2nd order" = "2nd order",
                   "Sequential" = "Sequential")

  # Plotting
  time_plot_p_varies <- ggplot(df_results_p_varies, aes(x = Features, y = Time, color=Method, shape = Method)) +
    geom_point(size = 2) +
    geom_line() +
    scale_color_manual(values = custom_colors,labels = custom_labels) +  # Custom colors
    scale_shape_manual(values = c(16, 17, 15, 18), labels = custom_labels) +  # Custom shapes and labels
    #(limits = c(0, 100), breaks = seq(0,100, by=10))+
    #scale_x_continuous(breaks = results_power$Censoring) +
    labs(x = "Number of features", y = "Time (min)") +
    theme_light() +
  ...
  vec_p_nonDTLCKe_time
  vec_p_parDTLCKe_time
  vec_p_second_order_time
  vec_p_sequential_time  
  
# Plotting
time_plot_n_varies <- ggplot(df_results_n_varies, aes(x = Features, y = Time, color=Method, shape = Method)) +
    geom_point(size = 2) +
    geom_line() +
    scale_color_manual(values = custom_colors,labels = custom_labels) +  # Custom colors
    scale_shape_manual(values = c(16, 17, 15, 18), labels = custom_labels) +  # Custom shapes and labels
    #(limits = c(0, 100), breaks = seq(0,100, by=10))+
    #scale_x_continuous(breaks = results_power$Censoring) +
    labs(x = "Number of samples", y = "Time (min)") +
    theme_light() +
    theme(plot.title = element_text(size = 18, hjust = 0.5))
...
vec_n_nonDTLCKe_time
vec_n_parDTLCKe_time
vec_n_second_order_time
vec_n_sequential_time
