...
uniform_to_original <- function(X, u_Xk, column_type){
  ...
  for(i in 1:p) {   
    if (column_type[i]=="num"){
      #Xk[,i] <- as.vector(quantile(X[,i], probs = u_Xk[,i], type=8))
...
# Correlation matrix
rho <- 0.8 # AR1 Correlation
corMat <- round(toeplitz(rho^(0:(p-1))), 10)

# Extracting the lower triangular part of the matrix that will be used in
# the gen_data() function from latencor package
lowerpart <- lower.tri(corMat)
rhos <- corMat[lowerpart] 
...
# Column names for the simulated data
df_column_names <- c("Ordinals_Proportion", "Iteration", "y", X_column_names)
df_column_names_betas <- c("Ordinals_Proportion", "Iteration", beta_column_names)
...
for (prop_ord in vec_prop_ord) {
  for (i in 1:n_sim) {
    ...
    p_dis <- round(p * prop_ord)
    p_con <- p - p_dis
    
    # Variable types for simulations
    ord <- rep("bin", p_dis)
    con <- rep("con", p_con)
    types <- sample(c(ord, con))
    
    # Mixed data type simulation: gen_data() function from latencor package
    X_mixed <- gen_data(n = n, types = types, 
                        rhos = rhos, copulas = "no", 
                        XP = NULL, showplot = FALSE)$X
    ...
    # Store each simulation as elements in the lists
...
#write.csv(df_nondvine_results, file = "df_nondvine_results_Gaussian_X.csv", row.names = FALSE)
...
#Exporting the results to csv file
#write.csv(df_dvine_results, file = "df_dvine_results_Gaussian_X.csv", row.names = FALSE)
...
#Exporting the results to csv file
#write.csv(df_second_order_results, file = "df_second_order_results_Gaussian_X.csv", row.names = FALSE)
...
#Exporting the results to csv file
#write.csv(df_lasso_results, file = "df_lasso_results_Gaussian_X.csv", row.names = FALSE)

results_lasso <- df_lasso_results %>% group_by(Ordinals_Proportion) %>%
  summarise(meanPower = mean(Power),meanFDP = mean(FDP) )
View(results_lasso)
...
for (idx in 1:n_simulations) {
  ...
  #Estimated coefficients for X
  cvfit <- cv.ncvreg(X=X, y=y,family=family_type, penalty = penalty_type, cluster=cl, nfolds=n_folds)
  fit <- ncvreg(X=X, y=y,family=family_type, penalty = penalty_type, cluster=cl, lambda=cvfit$lambda.min)
  Z <- coef(fit)[-1] #Not including the intercept
...
#Exporting the results to csv file
write.csv(df_SCAD_results, file = "df_SCAD_results_Gaussian_X.csv", row.names = FALSE)

results_SCAD <- df_SCAD_results %>% group_by(Ordinals_Proportion) %>%
  summarise(meanPower = mean(Power),meanFDP = mean(FDP) )
View(results_SCAD)
...
#Exporting the results to csv file
#write.csv(df_sequential_results, file = "df_sequential_results_Gaussian_X.csv", row.names = FALSE)
