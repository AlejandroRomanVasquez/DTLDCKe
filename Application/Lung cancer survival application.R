...
# stable_lasso_glmnet()--> Function to fit a regularized lasso regression model using 
#some functions of the R package glmnet.
#It implements the stabilizing procedure of Roberts and Nowak (2014) to diminish sensitivity
#to the fold assignment used in cross-validation to select the hyperparameter lambda
...
#Value: This function returns a vector of the estimated coefficients (without the intercept)
...
Lung_data_initial <- read.csv("Patients_final_506_Shedden.csv")
...
#Observations have a survival time equal to zero, which causes a problem when fitting the Cox-lasso model.
...
#2) The Lung_data contains the most 1000 expressed genes of a total of 12,259 genes of the
...
#The first 2 columns are related to survival, the following 3 are clinical, 
#and the remaining are the most expressed genes. 

Lung_data <- Lung_data_complete[, 1:(gen_p + 3 + 2)]
dim(Lung_data)
#[1] 292 155

# Calculate censoring rate for 'Pat_Died'
...
# Filter column names based on rejections equal to 1
...
# D-vine fitting
dvine_distributions <- X_Xk_dvine_distributions(X_cont = X_cont_dvine_order,column_type, par_vinecop_family, n_cores)
...
#### Fitting the Cox’s proportional hazards model with Lasso ####

# Initial setup
n_cores <- detectCores() - 2 # Parallel computing
cl <- makeCluster(n_cores)
registerDoParallel(cl) # Registering a parallel backend for doParallel

M_lasso <- 10
lasso_family <- 'cox'
n_folds <- 10 # Number of folds for Lasso CV

start_time <- Sys.time()
coef <- stable_lasso_glmnet_parallel(X, y_surv, lasso_family, M_lasso , n_folds , random_cv=TRUE)
end_time <- Sys.time()
print("Time for running the Lasso Model:")
print(end_time-start_time)
stopCluster(cl) #Stopping cluster

# Vector of integers that indicates the selected non-nulls position 
filtered_by_lasso <- which(coef!=0)
print(paste0("The number of selected variables is: ", length(filtered_by_lasso)))
...
# Filter column names based on rejections equal to 1
selected_columns_lasso <- colnames(X)[rejections_lasso == 1]
print(selected_columns_lasso)




#### Fitting the Cox’s proportional hazards model with SCAD ####
...
start_time <- Sys.time()
cvfit <- cv.ncvsurv(X=X, y=y_surv, penalty = penalty_type, cluster=cl, nfolds=n_folds)
fit <- ncvsurv(X=X, y=y_surv, penalty = penalty_type, cluster=cl, lambda=cvfit$lambda.min)
Z <- coef(fit) #There is not intercept to exclude
...
print(paste0("The number of selected variables is: ", length(just_rejections_SCAD)))
...
# Filter column names based on rejections equal to 1
selected_columns_SCAD <- colnames(X)[rejections_SCAD == 1]
